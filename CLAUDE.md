...
- **`record.viewport` is what the video contains, and its height is load-bearing.** Puppeteer captures exactly the viewport: no scrolling, no letterboxing, so anything below the fold is absent and nothing in the run says so. The default is `1280x1600`, taller than a screen on purpose — at `720` a real recording cut the page above the list the tests asserted on, and only watching the video revealed it. Both axes must stay **even**, because the H.264 conversion uses `yuv420p`.
