## <small>1.6.0-beta.1 (2026-09-06)</small>
...
**This is a prerelease.** `npm install twd-cli` keeps resolving to 1.5.0;
`npm install twd-cli@beta` gets this one. It ships as a beta for two reasons:
`twd-js` 1.10.0 is not published yet, and the viewport change above affects every
run, including runs that never touch a layout snapshot. Unlike 1.5.0, this
release cannot claim there is no behaviour change without the new flags.
